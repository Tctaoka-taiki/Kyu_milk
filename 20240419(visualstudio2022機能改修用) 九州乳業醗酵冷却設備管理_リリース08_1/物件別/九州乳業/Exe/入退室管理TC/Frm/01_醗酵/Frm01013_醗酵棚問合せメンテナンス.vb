...
Public Class Frm01013_醗酵棚問合せメンテナンス
    '#####################################################################################
    '型
    '#####################################################################################
    ...
    Private Sub Frm01013_醗酵棚問合せメンテナンス_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        倉庫情報表示()
    End Sub
    ...
    Private Sub 倉庫情報表示()
        ...
        Try
            With New CSqlEx
                ...
                .gSubSelect("MAX(B.所定醗酵時間) as 所定時刻")
                .gSubSelect("MAX(DATEDIFF(MINUTE,醗酵開始時刻,GETDATE())) as 経過時刻")
                .gSubFrom("DNトラッキング A")
                .gSubFrom("DM品種 B")
                '---------------------------------
                '醗酵倉庫内のトラッキングを対象とする
    ...
    Private Sub btnF10_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnF10.Click
            ...
            Try
                With New CSql
                    ...
                    If Me.dgv生産データ.Item(1, i).Value.ToString <> "" Then
                        ...
                        .gSubColumnValue("醗酵開始時刻", Me.dgv生産データ.Item(6, i).Value.ToString, True)
                        .gSubColumnValue("所定醗酵時間", Me.dgv生産データ.Item(7, i).Value.ToString, True)
                        .gSubColumnValue("醗酵経過時間", Me.dgv生産データ.Item(8, i).Value.ToString, True)
                    End If
        ...
        With New CSqlEx
            ...
            CMdiMain.帳票出力(New 棚在庫実績_醗酵, "***棚在庫実績（醗酵）***", .gSQL文の取得, "")
        End With
