...
Public Class Frm00003_醗酵側設備状況
    ...
    Private Sub 設備状況更新()
        ...
        Try
            ...
            '段積機
            信号情報更新_ライン別(0, デバイスOFFSET + 1, 0)
            荷情報更新_ライン別(0, デバイスOFFSET + 1, 2, 0)

            'コンベヤ
            信号情報更新_ライン別(0, デバイスOFFSET + 2, 1)

            荷情報更新_ライン別(0, デバイスOFFSET + 2, 4, 3)
            荷情報更新_ライン別(0, デバイスOFFSET + 2, 3, 4)
            荷情報更新_ライン別(0, デバイスOFFSET + 2, 2, 5)

            '入庫ST
            信号情報更新_ライン別(0, デバイスOFFSET + 3, 6)
            信号情報更新_ライン別(0, デバイスOFFSET + 3, 7)
            信号情報更新_ライン別(0, デバイスOFFSET + 3, 8)

            荷情報更新_ライン別(0, デバイスOFFSET + 3, 4, 6)
            荷情報更新_ライン別(0, デバイスOFFSET + 3, 3, 7)
            荷情報更新_ライン別(0, デバイスOFFSET + 3, 2, 8)
    ...
    Private Sub トラッキング情報更新(ByVal 製造ライン As Integer)
        Dim i As Integer = 0
        For i = 0 To 8
                ...
                If CUsrctl.gDp.gBlnReader(.gSQL文の取得, reader) Then
                    While reader.Read
                        Select Case reader.GetValue(0)
                            Case 0  '段積機データ
                                Me.dgv設備状況.Item(4, 0).Value = reader.GetValue(1)
                                Me.dgv設備状況.Item(5, 0).Value = reader.GetValue(2)
                                Me.dgv設備状況.Item(6, 0).Value = reader.GetValue(3)

                            Case 1  'コンベヤデータ ※複数データがストックされる場合がある
                                Me.dgv設備状況.Item(4, 5 - コンベヤCNT).Value = reader.GetValue(1)
                                Me.dgv設備状況.Item(5, 5 - コンベヤCNT).Value = reader.GetValue(2)
                                Me.dgv設備状況.Item(6, 5 - コンベヤCNT).Value = reader.GetValue(3)
                                コンベヤCNT += 1
                            Case 2
                                Select Case reader.GetValue(4)
                                    Case 3
                                        Me.dgv設備状況.Item(4, 8).Value = reader.GetValue(1)
                                        Me.dgv設備状況.Item(5, 8).Value = reader.GetValue(2)
                                        Me.dgv設備状況.Item(6, 8).Value = reader.GetValue(3)
                                    Case 2
                                        Me.dgv設備状況.Item(4, 7).Value = reader.GetValue(1)
                                        Me.dgv設備状況.Item(5, 7).Value = reader.GetValue(2)
                                        Me.dgv設備状況.Item(6, 7).Value = reader.GetValue(3)
                                    Case 1
                                        Me.dgv設備状況.Item(4, 6).Value = reader.GetValue(1)
                                        Me.dgv設備状況.Item(5, 6).Value = reader.GetValue(2)
                                        Me.dgv設備状況.Item(6, 6).Value = reader.GetValue(3)
                                End Select
                            ...
                            Case 3 'クレーン指示予約
                                Select Case reader.GetValue(4)
                                    Case 3
                                        ...
                                        Me.dgv設備状況.Item(5, 8).Value = reader.GetValue(2)
                                        Me.dgv設備状況.Item(6, 8).Value = reader.GetValue(3)
                                        Me.dgv設備状況.Item(7, 8).Value = reader.GetValue(5) & "-" & reader.GetValue(6).ToString.PadLeft(2, "0"c) & "-" & reader.GetValue(7)
                                    Case 2
                                        Me.dgv設備状況.Item(4, 7).Value = reader.GetValue(1)
                                        Me.dgv設備状況.Item(5, 7).Value = reader.GetValue(2)
                                        Me.dgv設備状況.Item(6, 7).Value = reader.GetValue(3)
    ...
    Private Sub 信号情報更新_ライン別(ByVal 倉庫区分 As Integer, ByVal デバイスNo As Integer, ByVal Row As Integer)
    ...
    Private Sub 荷情報更新_ライン別(ByVal 倉庫区分 As Integer, ByVal デバイスNo As Integer, ByVal ビットNo As Integer, ByVal Row As Integer)
